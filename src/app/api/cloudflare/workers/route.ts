...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ workers: res.result });
  } catch (err) {
...
export async function DELETE(request: Request) {
  ...
  if (!connectionId || !scriptName)
    return NextResponse.json({ error: "Missing connectionId or scriptName" }, { status: 400 });

  try {
    const client = await getClient(env, session.user.id, connectionId);
    await client.workers.delete(scriptName);
